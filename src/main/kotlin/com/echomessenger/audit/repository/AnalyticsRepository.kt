...
class AnalyticsRepository(
    ...
    /**
     * Читает из materialized views — не сканирует сырые таблицы.
     * Кэш 5 минут через Caffeine (@Cacheable).
     */
    ...
    fun getSummary(
        ...
        // Events by type из mv_daily_msg_type_stats
        val eventsByType =
            jdbc
                .query(
                    """
                ...
                .associate { (type, count) ->
                    AuditRepository.mapMsgTypeToEventType(type) to count
                }

        // Top active users из mv_daily_user_activity
        val topUsers =
            jdbc.query(
                """
                ...
                UserActivity(userId = rs.getString("usr_id"), count = rs.getLong("total"))
            }

        // По часам — только за последние 24h если диапазон <= 2 дня
        val byHour =
            jdbc.query(
                """
                ...
                HourlyBucket(hour = rs.getInt("hour"), count = rs.getLong("cnt"))
            }
    ...
    /**
     * Timeseries данные по конкретной метрике.
     */
    ...
    fun getTimeseries(
        ...
        return when (interval.lowercase()) {
            ...
            "day" -> {
                ...
                throw IllegalArgumentException("Unsupported interval: $interval. Use 'hour' or 'day'")
            }
        }
    }

    private fun mapMetricToMsgType(metric: String): String =
        when (metric) {
            "message.create" -> "PUB"
            "message.delete" -> "DEL"
            "auth.login" -> "LOGIN"
